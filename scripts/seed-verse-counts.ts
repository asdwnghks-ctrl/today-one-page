...
// Verse count per chapter, standard English versification (matches NWT for the
// overwhelming majority of chapters). Index 0 = chapter 1.
const VERSE_COUNTS_BY_BOOK: Record<string, number[]> = {
...
async function main() {
  const { data: books, error: booksError } = await supabase.from("books").select("id,chapter_count");
  if (booksError) throw new Error(booksError.message);
  const chapterCountByBook = new Map((books ?? []).map((b) => [b.id, b.chapter_count]));

  if (chapterCountByBook.size !== Object.keys(VERSE_COUNTS_BY_BOOK).length) {
    throw new Error(`Book count mismatch: DB has ${chapterCountByBook.size}, data has ${Object.keys(VERSE_COUNTS_BY_BOOK).length}`);
  }

  const rows: { book_id: string; chapter: number; verse_count: number }[] = [];
  let totalChapters = 0;

  for (const [bookId, verseCounts] of Object.entries(VERSE_COUNTS_BY_BOOK)) {
    const expected = chapterCountByBook.get(bookId);
    if (expected === undefined) throw new Error(`Unknown book id "${bookId}"`);
    if (verseCounts.length !== expected) {
      throw new Error(`Book "${bookId}" has ${verseCounts.length} chapters in the data but expected ${expected}`);
    }
    verseCounts.forEach((verseCount, index) => {
      rows.push({ book_id: bookId, chapter: index + 1, verse_count: verseCount });
    });
  ...
  if (segmentCountError) throw new Error(segmentCountError.message);
  if (segmentCount !== totalChapters) {
    throw new Error(`Data covers ${totalChapters} chapters but segments table has ${segmentCount}`);
  }

  console.log(`Validated ${rows.length} chapters across ${chapterCountByBook.size} books.`);

  const { error: insertError } = await supabase.from("verse_counts").upsert(rows, { onConflict: "book_id,chapter" });
  if (insertError) throw new Error(insertError.message);
  console.log(`Inserted/updated ${rows.length} rows into verse_counts.`);
}
